...
struct DecoderState {
    ...
    video_queue: VecDeque<VideoOutput>,
    audio_queue: VecDeque<AudioOutput>,
    last_video: Option<VideoOutput>,
        ...
        let result = decoder.commit_reserved(len, |output| {
            match output {
                DecodedOutput::Video {
                    ...
                    video_queue.push_back(VideoOutput {
                        stream_id,
                        pts,
                        frame_id,
                        frame,
...
/// Finalizes the current complete-file or standalone-epoch input transaction without consuming the handle.
#[unsafe(no_mangle)]
pub extern "C" fn decoder_finish_input(handle: u32) -> i32 {
    with_state(handle, |s| match s.decoder.finish_input() {
        Ok(()) => {
            s.error.clear();
            0
        }
...
fn with_stream<R: Copy>(
    ...
    index: u32,
    empty: R,
    f: impl FnOnce(&avelune::container::v1::StreamDesc) -> R,
) -> R {
    with_state(handle, |s| {
        s.decoder
            .front_index()
...
fn with_epoch<R: Copy>(
    ...
    index: u32,
    empty: R,
    f: impl FnOnce(&avelune::container::v1::EpochIndex) -> R,
) -> R {
    with_state(handle, |s| {
        s.decoder
            .front_index()
...
fn with_video<R: Copy>(handle: u32, empty: R, f: impl FnOnce(&VideoOutput) -> R) -> R {
    with_state(handle, |s| s.last_video.as_ref().map_or(empty, f)).unwrap_or(empty)
}
/// Stream identifier of the last popped video frame.
#[unsafe(no_mangle)]
pub extern "C" fn video_stream_id(handle: u32) -> u32 {
    with_video(handle, 0, |v| u32::from(v.stream_id))
}
/// Presentation timestamp of the last popped video frame.
#[unsafe(no_mangle)]
pub extern "C" fn video_pts(handle: u32) -> u64 {
    with_video(handle, 0, |v| v.pts)
}
/// Last popped video frame ID.
#[unsafe(no_mangle)]
pub extern "C" fn video_frame_id(handle: u32) -> u64 {
    with_video(handle, 0, |v| v.frame_id)
}
/// Last popped video width.
#[unsafe(no_mangle)]
pub extern "C" fn video_width(handle: u32) -> u32 {
    with_video(handle, 0, |v| v.frame.width)
}
/// Last popped video height.
#[unsafe(no_mangle)]
pub extern "C" fn video_height(handle: u32) -> u32 {
    with_video(handle, 0, |v| v.frame.height)
}
/// Last popped Y plane pointer.
#[unsafe(no_mangle)]
pub extern "C" fn video_y_ptr(handle: u32) -> usize {
    with_video(handle, 0, |v| v.frame.y().as_ptr() as usize)
}
/// Last popped U plane pointer.
#[unsafe(no_mangle)]
pub extern "C" fn video_u_ptr(handle: u32) -> usize {
    with_video(handle, 0, |v| v.frame.u().as_ptr() as usize)
}
/// Last popped V plane pointer.
#[unsafe(no_mangle)]
pub extern "C" fn video_v_ptr(handle: u32) -> usize {
    with_video(handle, 0, |v| v.frame.v().as_ptr() as usize)
}
/// Last popped Y byte length.
#[unsafe(no_mangle)]
pub extern "C" fn video_y_len(handle: u32) -> u32 {
    with_video(handle, 0, |v| v.frame.y().len() as u32)
}
/// Last popped U/V byte length.
#[unsafe(no_mangle)]
pub extern "C" fn video_uv_len(handle: u32) -> u32 {
    with_video(handle, 0, |v| v.frame.u().len() as u32)
}

/// Stream identifier of the last popped audio packet.
#[unsafe(no_mangle)]
pub extern "C" fn audio_stream_id(handle: u32) -> u32 {
    with_state(handle, |s| {
        s.last_audio.as_ref().map_or(0, |a| u32::from(a.stream_id))
    })
    .unwrap_or(0)
}
...
pub extern "C" fn kernel_halfpel_predict_probe(phase: u32) -> u64 {
    ...
    let Some(predicted) =
        avelune::kernels::KernelSet::auto().halfpel_predict_8x8(&reference, 16, fx, fy)
    else {
        return u64::MAX;
    };
    predicted
...
pub extern "C" fn kernel_halfpel_sad_probe(phase: u32) -> u64 {
    ...
    let fx = (phase & 1) as u8;
    let fy = ((phase >> 1) & 1) as u8;
    avelune::kernels::KernelSet::auto()
        .halfpel_sad_8x8(&source, 16, &reference, 16, fx, fy)
        .unwrap_or(u64::MAX)
}

/// Runs the selected runtime SAD kernel on a deterministic validation vector.
...
pub extern "C" fn kernel_sad_probe() -> u64 {
    ...
    avelune::kernels::KernelSet::auto().sad(&a, &b)
}
