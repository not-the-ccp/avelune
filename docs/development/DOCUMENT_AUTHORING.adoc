...
Files under `docs/` and `spec/` are authored in AsciiDoc and rendered by the custom Astro publication system. Checked-in source remains authoritative; generated HTML is a presentation artifact. The publication loader builds one validated content graph for routes, navigation, source provenance, headings, and cross-references. This profile deliberately keeps the language small enough that a contributor can understand a document by opening one file.
...
Allowed `page-type` values are `guide`, `architecture`, `development-guide`, `normative`, `supporting`, and `historical`. Normative documents use `page-status: current normative draft · unfrozen`. Historical specification material uses `page-type: historical` and `page-status: historical · non-normative`. `nav-order` is an integer used only for navigation order. Public routes are derived centrally from source identity and validated for collisions; documents must not hardcode rendered routes into prose.
...
Run `npm run check:content` while authoring and `./scripts/build-site.sh --skip-wasm` before review. Validation checks metadata, IDs, xrefs, local assets, stale Markdown destinations, normative restrictions, historical classification, and publication-route collisions. Generated-site validation checks both files and fragments. For navigation, responsive, or interactive changes, also run the browser site smoke when Chromium is available; an unavailable browser is reported as a skip.
