...
:summary: A historical, non-normative record of the internal Avelune POC snapshot that was once labeled 1.0 and is now superseded.
:nav-order: 200

[NOTE]
====
This document is historical and non-normative. It records an internal POC snapshot that was prematurely labeled “1.0”. The public project now treats these formats as unfrozen Draft Generation 1.
====
...
[#history-v1-implementations-tools]
...
[#history-v1-real-media-benchmarks]
...
[#history-v1-security-interoperability]
...
[#history-v1-known-limitations]
== Known limitations

V1 Baseline intentionally does not define 4:2:2, 4:4:4, >8-bit video, alpha, or baseline multichannel audio. Hand-written SIMD is not shipped. Scalar WASM falls below 1080p30 on the local Node/V8 fixture. WebGPU codec compute remains experimental. Release fuzzing is deterministic mutation smoke testing rather than a long coverage-guided campaign. No claim of patent freedom is made; see xref:../IPR-NOTES.adoc[IPR notes].
