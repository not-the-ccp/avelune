...
:summary: Normative Draft Generation 1 integer byte order, canonical varints, and static byte rANS entropy coding.
:nav-order: 40
:sectnums:
...
A conforming entropy block uses mode 0 or mode 2. All other mode values are invalid.
...
. `x = state & 4095`.
. `s = symbol_at[x]`.
. `state = F[s] * (state >> 12) + (x - C[s])`.
. While `state < L`, consume one coded byte `b` and set `state = (state << 8) | b`.
. Output `s`.

The decoder MUST consume exactly `coded_len` bytes after producing `raw_len` bytes.

[#entropy-non-normative-encoder-behavior]
