...
The canonical Rust decoder and the independent scalar decoder in `avelune-reference` are informative implementations. Neither source tree is normative. The independent encoder in `avelune-reference` is used only for differential testing: interoperability is checked by decoding its packets with the canonical decoder and comparing the decoded reconstructions, not by requiring byte-identical packets from encoders whose policy choices may legitimately differ.
