#!/usr/bin/env python3
"""Generate reproducible reference-vs-production ALV1 size/quality curves.
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="avelune-curves-") as td_s:
        ...
        run([str(cli), "--backend", "prod", "raw", "decode-y4m", str(demo), str(source)])
        for q in qs:
            for backend in ("reference", "prod"):
                encoded = td / f"{backend}-q{q}.avl"
                decoded = td / f"{backend}-q{q}.y4m"
                durations: list[float] = []
                for _ in range(args.repeats):
                    start = time.perf_counter()
                    run([
                        str(cli), "--backend", backend, "raw", "encode-y4m",
                        str(source), str(encoded), "--q", str(q), "--preset", "balanced"
                    ])
                    durations.append(time.perf_counter() - start)
                run([str(cli), "--backend", "prod", "raw", "decode-y4m", str(encoded), str(decoded)])
                ...
                print(
                    f"{backend:9s} q={q:<3d} bytes={encoded.stat().st_size:<8d} "
    ...
    with out_csv.open("w", newline="") as f:
        fields = ["backend", "q", "bytes", "encode_seconds_median", "psnr_average_db", "lossless_psnr", "ssim_all"]
