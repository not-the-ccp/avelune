...
cargo test -p avelune-kernels --release --locked --no-run >/tmp/avelune-kernel-build.log 2>&1
bin=$(find target/release/deps -maxdepth 1 -type f -name 'avelune_kernels-*' -perm -111 | head -n1)
...
{
  echo '# Avelune canonical instruction-selection evidence'
...
if rustc --print target-libdir --target wasm32-unknown-unknown >/dev/null 2>&1 \
  ...
  ./scripts/build-wasm.sh >/tmp/avelune-wasm-build.log 2>&1
  ...
  if [[ -n $llvm_objdump ]]; then
    "$llvm_objdump" -d web/player/avelune-simd128.wasm > "$wasm_dump"
...
echo 'disassembly inspection PASS'
