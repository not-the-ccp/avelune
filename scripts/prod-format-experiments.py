...
with reproducible measurements cheaply enough to run in the production validation workflow.
"""
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='avelune-format-exp-') as td:
        ...
        subprocess.run([a.bin,'--backend','prod','raw','decode-y4m',a.media,str(y4m)],check=True,stdout=subprocess.DEVNULL)
        w,h,frames=parse_y4m(y4m)
        subprocess.run([a.bin,'--backend','prod','raw','encode-y4m','-q','96',str(y4m),str(q96)],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
        subprocess.run([a.bin,'--backend','prod','raw','decode-y4m',str(q96),str(q96_y4m)],check=True,stdout=subprocess.DEVNULL)
