...
use avelune_reference::{
    audio as ref_audio, video_decoder as ref_video_decoder, video_encoder as ref_video_encoder,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut json_path = PathBuf::from("results/canonical-bench.json");
    let mut csv_path = PathBuf::from("results/canonical-bench.csv");
    ...
    let f = canonical_frame(640, 360);
    let rf = ref_encoder_frame(&f);
    ...
    let ropts = ref_video_encoder::EncodeOptions {
        qstep: 73,
        motion_radius: 0,
        max_refs: 0,
        allow_palette: true,
    ...
    samples.push(measure(
        ...
        "video.encode.canonical.scalar.single.640x360.q73",
        video_iters,
        1,
        f.storage_len(),
    ));
    ...
    samples.push(measure(
        ...
        "video.encode.canonical.auto.single.640x360.q73",
        video_iters,
        1,
        f.storage_len(),
    ));
    ...
    samples.push(measure(
        ...
        "video.encode.canonical.auto.private_pool.640x360.q73",
        video_iters,
        1,
        f.storage_len(),
    ));
    samples.push(measure(
        ...
        "video.encode.canonical.scoped_threads.640x360.q73",
        video_iters,
        1,
        f.storage_len(),
    ));
    samples.push(measure(
        || {
            let e = ref_video_encoder::encode(1, black_box(&rf), &[], ropts).unwrap();
            e.packet.len() as u64 ^ u64::from(e.reconstructed.y[0])
        },
    ...
    let inter_ropts = ref_video_encoder::EncodeOptions {
        qstep: 73,
        motion_radius: 4,
        max_refs: 1,
        allow_palette: true,
    ...
    for (name, cfg) in [
        (
            "video.encode.inter.canonical.scalar.single.640x360",
            single_scalar,
        ),
        (
            "video.encode.inter.canonical.auto.single.640x360",
            single_auto,
        ),
    ] {
    ...
    samples.push(measure(
        || {
            let refs = [(1_u64, &rf)];
            let e = ref_video_encoder::encode(2, black_box(&rf2), &refs, inter_ropts).unwrap();
            e.packet.len() as u64 ^ u64::from(e.reconstructed.y[0])
        },
    ...
    samples.push(measure(
        || {
            ...
            let (_, d, _) = scalar_decoder
                .decode(black_box(&canonical_encoded.packet))
                .unwrap();
            d.y().len() as u64 ^ u64::from(d.y()[0])
        },
        "video.decode.canonical.scalar.single.640x360.q73",
        4 * scale,
        1,
        canonical_encoded.packet.len(),
    ));
    ...
    samples.push(measure(
        ...
        "video.decode.canonical.auto.single.640x360.q73",
        4 * scale,
        1,
        canonical_encoded.packet.len(),
    ));
    ...
    samples.push(measure(
        ...
        "video.decode.canonical.auto.private_pool.640x360.q73",
        4 * scale,
        1,
        canonical_encoded.packet.len(),
    ));
    samples.push(measure(
        || {
            let (_, d, _) = video::decode(black_box(&canonical_encoded.packet), &[]).unwrap();
            d.y().len() as u64 ^ u64::from(d.y()[0])
        },
        "video.decode.canonical.scoped_threads.640x360.q73",
        4 * scale,
        1,
        canonical_encoded.packet.len(),
    ));
    samples.push(measure(
        || {
            let (_, d, _) =
                ref_video_decoder::decode(black_box(&canonical_encoded.packet), &[]).unwrap();
            d.y.len() as u64 ^ u64::from(d.y[0])
        },
    ...
    samples.push(measure(
        ...
        "audio.encode.canonical.4096x2.q37",
        10 * scale,
        2,
        pcm.len() * 2,
    ));
    ...
    samples.push(measure(
        ...
        "audio.decode.canonical.4096x2.q37",
        15 * scale,
        2,
        audio_packet.len(),
    ));
    ...
    samples.push(measure(
        ...
        "audio.decode_into.canonical.reuse.4096x2.q37",
        15 * scale,
        2,
        audio_packet.len(),
    ));
